import { array1 } from './templates.js'
...
const inputText = new Set(Array.from(templateShower.querySelectorAll(`input[type="text"]`)))

const inputRadio = new Set(Array.from(templateShower.querySelectorAll(`input[type="radio"]`)))

const primaryContact = new Set(Array.from(templateShower.querySelectorAll(`input[type="number"]`)))
...
function fcFunction () {
 ...
 const FCdetailed =  document.getElementById("FCdetailed").innerHTML
    ...
    const inputRadioName = Array.from(inputRadio).map((radioName) => radioName.name)

    const inputTextValue = Array.from(inputText).map((input) => input.value)


    const primaryContactValue = Array.from(primaryContact).map((number) => number.value)
    ...
    for (let i = 0; i < FCradioArr.length; i += 2) {
        const testes = FCradioArr[i]
        radioFinalVal.push(testes)
    }
    
    // console.log(radioFinalVal)
   para.innerHTML = ` 
